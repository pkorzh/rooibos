		...
		pre: function(node, parent, prop, idx) {
			_registrator.patterns.forEach(function(pattern) {
				if (node.match(pattern.ast)) {
					var matchedNode = pattern.callback.call(null, node, parent, rootScope);
					if (matchedNode !== null) {
						debugger;
						var code = escodegen.generate(matchedNode).split('\n').splice(0,3).map(function(chunk) {
							return '\t' + chunk;
						}).join('\n');
...
function _registrator(module) {
	return function(desc, ast, callback) {
		_registrator.patterns.push({
			desc: desc,
			ast: ast,
			callback: callback || function(node, parent, rootScope) {
				return node;
			},
			module: module
		});
